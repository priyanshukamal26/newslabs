...
- **Sentiment (VADER-style)**: Analyzes positive/negative word ratios, handling negations ("not good") to determine if an article is Positive, Neutral, or Negative.
...
- **Article Selection Algorithm**: Before sending a brief, the system queries the in-memory store for articles from the last 24 hours. It ranks them using a composite score:
...
**Q: How does the Naive Bayes / Logistic Regression NLP work in plain TypeScript?**
